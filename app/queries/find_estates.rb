class FindEstates
  ...
  def filter_by_id(scoped, id = nil)
    id ? scoped.where('estates.id = ?', id) : scoped
  end

  def filter_by_estate_project(scoped, estate_project_id = nil)
    estate_project_id ? scoped.where(estate_project_id: estate_project_id) : scoped
  end

  def filter_by_number_of_rooms(scoped, number_of_rooms = nil)
    number_of_rooms ? scoped.where('estates.number_of_rooms = ?', number_of_rooms) : scoped
  end

  def filter_by_floor(scoped, floor = nil)
    floor ? scoped.where('estates.floor = ?', floor) : scoped
  end

  def filter_by_price_to(scoped, price_to = nil)
    price_to ? scoped.where('estates.price <= ?', price_to) : scoped
  end

  def filter_by_responsible_employee(scoped, responsible_employee_id = nil)
    responsible_employee_id ? scoped.where(responsible_employee_id: responsible_employee_id) : scoped
  end
