namespace :import do
  ...
  task leadercom: :environment do
    ...
    IMPORT_ERRORS_FILE = 'tmp/import-errors.txt'

    DatabaseCleaner.clean_with(:truncation) unless Rails.env.production?

    city_id = City.find_or_create_by!(name: 'Нефтеюганск')
    ...
    STREETS.each do |street|
      s = Street.find_or_create_by!(city: city_id, name: street)
    ...
    ESTATE_TYPES.each do |key, value|
      estate_types_mapping[value] = EstateType.find_or_create_by!(name: value)
    end
    ...
    estate_project_not_found = EstateProject.create!(name: 'Неизвестен')

    estate_projects_mapping = {}
    ESTATE_PROJECTS.each do |key, value|
      estate_projects_mapping[value] = EstateProject.find_or_create_by!(name: value)
    end
    ...
    estate_material_not_found = EstateMaterial.create!(name: 'Неизвестен')

    estate_materials_mapping = {}
    ESTATE_MATERIALS.each do |key, value|
      estate_materials_mapping[value] = EstateMaterial.find_or_create_by!(name: value)
    end
    ...
    employees.each_with_index do |employee, idx|
      ...
      e.lock_access!(send_instructions: false)
    end
    ...
    invalid_estates = estates.map do |estate|
      ...
      estate[:building_number] = 'Не указан' if estate[:building_number].blank?
    ...
    unless invalid_estates.size.zero?
      ...
      invalid_estates_cnt = 0

      invalid_estates.each do |estate|
        next if estate[:message].blank?
        invalid_estates_cnt += 1
      ...
      if invalid_estates_cnt.positive?
    ...
    File.delete(IMPORT_ERRORS_FILE) if File.exists?(IMPORT_ERRORS_FILE)
    ...
    invalid_estates.each do |item|
      ...
      puts "--- Processing (#{cnt}/#{total_estates}) ---"

      street = item[:street].gsub(/^ул.\s+/, '').downcase
      street = case street
               when '1'
                 '1-й мкр'
               when '7'
      ...
      if !STREETS_DOWNCASED.include?(street)
        abort "Неизвестное название улицы '#{street}'. Как должно быть: '1-й мкр', '9А мкр', 'Спортивная'"
      end

      street   = Street.find_by_name(street)
      address  = Address.find_or_create_by!(street: street, building_number: item[:building_number])
      employee = Employee.find_by_id(employees_mapping[item[:employee]][:employee_id])

      item[:client_full_name]     = clients_mapping[item[:client]][:full_name]
      item[:client_phone_numbers] = clients_mapping[item[:client]][:phone_numbers]
      item[:address]              = address
      item[:created_by_employee]  = employee
      item[:responsible_employee] = employee
      item[:estate_type]          = estate_types_mapping[item[:estate_type]]
      item[:estate_material]      = estate_materials_mapping[item[:estate_material]] || estate_material_not_found
      item[:estate_project]       = estate_projects_mapping[item[:estate_project]] || estate_project_not_found
      item[:price]                = item[:price].to_i
      item[:created_at]           = item[:created_at].to_datetime
      item[:updated_at]           = item[:updated_at].to_datetime

      item.delete_if { |field| field.in?([:sid, :object_id, :client, :employee, :street, :building_number, :message]) }

      estate = Estate.new(item)
      if estate.valid?
        estate.save!
      else
        message = "\n--- В объекте ##{object_id} (#{sid}) обнаружены ошибки:---\n"
        message += estate.errors.full_messages.map { |m| "* #{m}" }.join("\n") + "\n"
        File.open(IMPORT_ERRORS_FILE, 'a') { |f| f.write(message) }
      end
    end

    if File.exists?(IMPORT_ERRORS_FILE)
      abort "В процессе импорта обнаружены ошибки!\nПосмотрите файл #{IMPORT_ERRORS_FILE}."
    end
  end
